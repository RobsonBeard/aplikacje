...
const profilesRouter = async (req, res) => {
  if (req.url === '/api/profile' && req.method === 'GET') { // pobranie jsona tablicy userow
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
      ...
      const updateProfileDataResponse = await profilesController.updateProfileData(token, parsedData)
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
      ...
      const addProfilePictureResponse = await profilesController.addProfilePicture(req, token)
