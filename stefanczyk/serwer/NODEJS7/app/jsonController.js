...
module.exports = {
  ...
  update: (modificationData) => {
    return new Promise((resolve, reject) => {
      try {
        if (imagesArr.length !== 0) {
          const selectedImage = imagesArr.filter((elem) => elem.id === modificationData.id) //* wyjdzie tablica
          if (selectedImage.length === 1) {
  ...
  addTag: () => {
    // TODO: patch taga, wyszukac zdjecie po ID, dorobic w ruterze ifa, spushowac do tablicy taga
  }
}
